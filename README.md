...
- **📄 Docs the Agent can fetch itself** — The instance serves its own reference over HTTP (`/api.md`) and registers the same bytes as an MCP resource (one URI, two access paths), with a downloadable curl skill (`/skills.md`) and a `learn-api` prompt — an Agent can adopt termcp over REST alone, with no MCP client installed.
...
- **🛡️ Single-token HTTP authentication** — One static token guards the entire HTTP surface: Web UI, REST API, MCP SSE, MCP streamable HTTP, and the WebSocket — except the read-only docs `/api.md` and `/skills.md` (no data inside), which stay fetchable without a token so agents/scripts can learn the API before they have one. Configure the token itself (`--auth-token` / `TERMCP_AUTH_TOKEN`) or only its salted SHA-256 hash (`--auth-hash` / `TERMCP_AUTH_HASH`, generated by `termcp --gen-auth-hash`) so the server never stores the plaintext. Non-loopback binds refuse to start without one.
