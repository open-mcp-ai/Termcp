...
- **REST 定位符解析 `GET /api/resolve?url=termcp://...`**：把 Web UI 复制的定位符在 HTTP 侧一次性解析成具体 id，纯 REST/curl 的 agent 不再需要自己实现语法解析。返回 `{"kind":"entry"|"session"|"shell", ...}`：entry → `ssh_config`；session → `session_id`/`name`/`status`；shell → `session_id`+`shell_id`+`index`（1 起，与 `shell-1`/`shell-2` 标签一致）。归档会话返回 `"status":"archived"`/`"archived":true`（只读提示）；错误语义：400 畸形定位符、404 未知 profile/会话/序号越界、409 对归档会话用 shell 定位符。定位符解析器抽到共享包 `internal/locator`（MCP `internal/mcp/resource_url.go` 改为薄适配层，两边同一份语法与测试）。
