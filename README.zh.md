...
- **🔍 审阅模式——Agent 写入前先问你** —— 可按会话开启，也可通过连接配置上的 `default_approval` 让某台主机默认开启。开启后 Agent 经 MCP 的每一次写入都先停下等一次裁决：终端输入（整条命令行，文本 + 结束它的那个键）、文件传输、端口转发。Web UI 队列里每条都有可读摘要（*「write 1.2 KB to /etc/hosts」*），计数挂在打开队列的那把锁上。**你自己的键盘和 Web UI 文件浏览器永不被拦**——闸门盖的是 Agent 的程序化面（MCP），不是人的交互面；只读操作同样不拦。
