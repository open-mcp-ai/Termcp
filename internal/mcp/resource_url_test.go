...
func TestParseResourceURL(t *testing.T) {
	cases := []struct {
		...
		// Entry locators
		{"entry bare", "mac", true, "", resourceURLEntry, "mac", "", 0},
		{"entry scheme", "termcp://mac", true, "", resourceURLEntry, "mac", "", 0},
		{"entry internal", "termcp://internal", true, "", resourceURLEntry, "internal", "", 0},
		{"entry trailing slash", "termcp://mac/", true, "", resourceURLEntry, "mac", "", 0},

		// Session locators (short form preferred)
		{"session short", "#abc123def", true, "", resourceURLSession, "", "abc123def", 0},
		{"session scheme short", "termcp://#abc123def", true, "", resourceURLSession, "", "abc123def", 0},
		{"session scheme short slash", "termcp://#/abc123def", true, "", resourceURLSession, "", "abc123def", 0},
		{"session session-prefix", "#session-abc123def", true, "", resourceURLSession, "", "abc123def", 0},
		{"session scheme session-prefix", "termcp://#session-abc123def", true, "", resourceURLSession, "", "abc123def", 0},
		{"session long form", "termcp://mac#abc123def", true, "", resourceURLSession, "", "abc123def", 0},
		{"session long form session-prefix", "termcp://mac#session-abc123def", true, "", resourceURLSession, "", "abc123def", 0},

		// Shell locators
		{"shell short index", "#abc123def:2", true, "", resourceURLShell, "", "abc123def", 2},
		{"shell short index 1", "#abc123def:1", true, "", resourceURLShell, "", "abc123def", 1},
		{"shell scheme short index", "termcp://#abc123def:3", true, "", resourceURLShell, "", "abc123def", 3},
		{"shell long form index", "termcp://mac#abc123def:2", true, "", resourceURLShell, "", "abc123def", 2},
		{"shell scheme short slash index", "termcp://#/abc123def:2", true, "", resourceURLShell, "", "abc123def", 2},
		{"session prefix with index", "#session-abc123def:2", true, "", resourceURLShell, "", "abc123def", 2},
		{"long form session prefix with index", "termcp://mac#session-abc123def:2", true, "", resourceURLShell, "", "abc123def", 2},
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if p.sid != tc.sid {
				t.Errorf("sid = %q, want %q", p.sid, tc.sid)
			}
			if p.index != tc.index {
